...
export const backendService = async (identity?: any) => {
  ...
  return {
    raw: {
      ...
      recordDocumentView: (documentId: string) =>
        actor.record_document_view(documentId) as Promise<BackendResult<null>>,
    ...
    async recordDocumentView(documentId: string): Promise<boolean> {
      const result = await this.raw.recordDocumentView(documentId);
      return isBackendSuccess(result);
    },
